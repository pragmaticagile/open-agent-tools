...
if (!fs.existsSync(serverPath)) {
  fail("dist/index.js was not found. Run `npm install` and `npm run build`, then run `npm run setup:claude` again.");
}
...
async function main() {
  console.log("jira-plus-mcp Claude Desktop setup");
  console.log("");
  console.log("This will add or update jira_plus in Claude Desktop config.");
  ...
  const existingEnv = existingJira?.env || {};
  ...
  const writeTools = writeModeAnswer.trim().toLowerCase().startsWith("y") ? "true" : "false";
  ...
  const env = {
    ...
    JIRA_EMAIL: email,
    JIRA_ENABLE_WRITE_TOOLS: writeTools
  };
  if (useTokenFile) {
    env.JIRA_API_TOKEN_FILE = tokenFile;
  } else {
    env.JIRA_API_TOKEN = token || existingEnv.JIRA_API_TOKEN || "YOUR_API_TOKEN_HERE";
  }

  config.mcpServers.jira_plus = {
    command: "node",
    args: [serverPath],
    env
  };
  ...
  if (!detailsNow) {
    console.log("0. Replace the placeholder Jira URL, email, and token in Claude Desktop config.");
  }
  console.log("1. Fully quit Claude Desktop.");
  console.log("2. Reopen Claude Desktop.");
  console.log("3. Ask Claude: Use jira_plus to run get_connection_status.");
  console.log("");
  console.log(`Write tools are ${writeTools === "true" ? "enabled" : "disabled/read-only"}.`);
}
...
function readJsonConfig(filePath) {
  ...
  try {
    ...
    fail(`Claude Desktop config is not valid JSON: ${filePath}\n${error.message}`);
  }
}

function validate(config) {
  const jira = config?.mcpServers?.jira_plus;
  if (!jira?.env?.JIRA_BASE_URL || !jira?.env?.JIRA_EMAIL || (!jira?.env?.JIRA_API_TOKEN && !jira?.env?.JIRA_API_TOKEN_FILE)) {
    fail("Setup wrote the config, but required Jira settings are missing. Re-run setup and provide Jira site, email, and token/token file.");
  }
}
...
function normalizeBaseUrl(value) {
  ...
  if (!trimmed) {
    return "https://YOUR_JIRA_SITE_NAME.atlassian.net";
  }
