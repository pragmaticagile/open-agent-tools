...
if (!fs.existsSync(serverPath)) {
  fail("dist/index.js was not found. Run `npm install` and `npm run build`, then run `npm run setup:vscode` again.");
}
...
async function main() {
  console.log("jira-plus-mcp VS Code setup");
  console.log("");
  console.log("This writes a workspace MCP config at .vscode/mcp.json by default.");
  console.log("A backup will be created before changes are written.");
  console.log("");

  const existing = readJsonConfig(configPath);
  const existingJira = existing?.servers?.jira_plus;
  const existingEnv = existingJira?.env || {};
  const details = await collectDetails(existingEnv);

  const config = isPlainObject(existing) ? existing : {};
  config.servers = isPlainObject(config.servers) ? config.servers : {};
  backupFile(configPath);
  config.servers.jira_plus = {
    type: "stdio",
    command: "node",
    args: [serverPath],
    env: details.env
  ...
  console.log("");
  console.log("VS Code MCP config updated successfully.");
  console.log(`Config: ${configPath}`);
  console.log("Next steps:");
  if (!details.detailsNow) console.log("0. Replace the placeholder Jira URL, email, and token in .vscode/mcp.json.");
  console.log("1. Restart or reload VS Code.");
  console.log("2. Ask your VS Code agent: Use jira_plus to run get_connection_status.");
}
...
function readJsonConfig(filePath) {
  ...
  catch (error) { fail(`VS Code MCP config is not valid JSON: ${filePath}\n${error.message}`); }
}
...
function validate(config) {
  ...
  if (!jira?.env?.JIRA_BASE_URL || !jira?.env?.JIRA_EMAIL || (!jira?.env?.JIRA_API_TOKEN && !jira?.env?.JIRA_API_TOKEN_FILE)) {
    fail("Setup wrote the config, but required Jira settings are missing.");
  }
}
